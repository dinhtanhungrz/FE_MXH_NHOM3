...
return `
<img src="${avatarUrl}" alt="${userName}" class="${size} rounded-full object-cover" />
`;
};
...
return `
<button onclick="${onClick}" class="px-4 py-2 rounded-lg font-medium transition ${styles[type] || styles.primary}">
    ${text}
</button>
`;
};
...
return `<span class="user-link cursor-pointer font-semibold text-gray-800 hover:underline ${extraClasses}"
    data-user-id="${user.id}">@${user.fullName || user.username || "User"}</span>`;
}
