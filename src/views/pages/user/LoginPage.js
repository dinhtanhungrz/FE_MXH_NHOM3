            ...
            <div class="max-w-md w-full">
                ...
                <div class="bg-white rounded-2xl shadow-xl p-8">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <!--<button id="google-login-btn" class="flex items-center justify-center px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
...
/**
 * Khởi tạo sự kiện cho trang Login
 */
export const initLoginPageEvents = () => {
    setupLoginHandlers();// Form đăng nhập thường
    initGoogleLogin(); 
...
function initGoogleLogin() {
    if (!window.google || !google.accounts) {
        console.error("Google script chưa load");
        return;
    }

    google.accounts.id.initialize({
        client_id: "94285014738-34uolut9duf83kc56ka4lu8c2ij1lll8.apps.googleusercontent.com",
...
/**
 * Handle Google response (nhận ID Token)
 */
async function handleCredentialResponse(response) {
    ...
    try {
        ...
        // ĐỊNH NGHĨA BIẾN result Ở ĐÂY
        const result = await res.json(); 

        if (res.ok && result.data) {
            const authData = result.data; // { accessToken, refreshToken, roles, email, username }
            ...
            // 2. Điều hướng dựa trên role (Dùng roles từ authData)
...
/**
 * Setup login form handlers - đăng nhập thường
 */
function setupLoginHandlers() {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            if (result.role && result.role.filter((r) => r === "ROLE_ADMIN").length > 0) {
                router.navigate("/admin");
            } else if (result.role && result.role.filter((r) => r === "ROLE_USER").length > 0) {
                router.navigate("/");
            } else {
